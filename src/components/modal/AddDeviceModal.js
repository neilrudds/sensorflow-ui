...
const AddDeviceModal = ({ onAdd }) => {
    ...
    const [formData, setFormData] = useState({ serial: "", name: "", location: "" });
    ...
    // Retrieve gateways to populate modal drop down list
    ...
    // Add new fields to the field table
    const createField = () => {
        setFields([
            ...fields,
    ...
    const handleSubmit = async () => {
        ...
        const nanoid = customAlphabet('1234567890abcdef', 10) // Use custom alphabet for a simpler device serial
        let deviceId = formData.serial;

        // If a user hasn't populated the serial no. generate one for them
        if (deviceId === "") {
            deviceId = nanoid();
        }

        const params = { id: deviceId, name: formData.name, location: formData.location, workspaceId: workspace.id, gatewayId: selectedGateway, fields: JSON.stringify(fields) };
        try {
            createDevice(params)
                .then((res) => {
                    console.log(res);
